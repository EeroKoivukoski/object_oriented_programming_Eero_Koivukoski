 ...
 * a class inheriting from DistributionLib.Generator accordingly. Use the return type to implements either
 * DistributionLib.DiscreteGenerator or DistributionLib.ContinuousGenerator. Prompt the user to specify constraints on the parameters.
 * @date 4/10/2002
 * @author F.Mallet
 */
public class MetaGenerator {
	...
	pw.println(" * Automatically generated by <code>DistributionLib.MetaGenerator</code>.");
	pw.println(" * @version 1.0, "+Calendar.getInstance().getTime());
	pw.println(" * @author F.Mallet, adaptation from C.Simatos's original");
	pw.println(" */\n");
	pw.print("public class "+className+" extends DistributionLib.Generator implements ");
	if (m.getReturnType().getName().equals("double"))
	    pw.print("Continuous");
	else pw.print("Discrete");
	pw.println("DistributionLib.Generator {");
	...
	pw.println("    /**");
	pw.println("     * the seed is aumatically provided by the <code>DistributionLib.SeedGenerator</code>");
	pw.println("     */");
	...
	for (int i=0; i<nbC; i++) {
	    ...
	    pw.println("        if ("+c+")");
	    pw.println("            throw new DistributionLib.ParameterException(\""+className+": "+err+".\");");
	}
	...
	if (args.length==0) {
	    System.err.println("Usage: java DistributionLib.MetaGenerator <distrib_name> [#<nb_constraints>]");
	    System.exit(1);
	}
	int nb = 0;
	try {

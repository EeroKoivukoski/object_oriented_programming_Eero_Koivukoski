...
/**
 * A random number generator based on the DistributionLib.Binomial distribution.
...
public class Binomial extends Generator implements DiscreteGenerator {
  ...
  private int trials;
    ...
    /**
     * the seed is automatically provided by the <code>DistributionLib.SeedGenerator</code>
	...
	if (prob <= 0.0)
	    throw new ParameterException("DistributionLib.Binomial: The probability of success must be between 0 and 1.");
	if (trials <= 0)
	    throw new ParameterException("DistributionLib.Binomial: The number of trials must be a positive integer.");
	this.prob = prob;
	this.trials = trials;
    }
    ...
    public String toString() { return "DistributionLib.Binomial("+prob+", "+trials+")"; }
}
